...
export class ContacsHtmlService {

  constructor(private stg: StorageService) { }


  createContactsListLetters(firstCha:any) {
    return /*html*/`
        <div>
  ...
  createPopupExistContactIcon(i: number) {
      return /*html*/`
          <div style="background-color:${this.stg.contacts[i]['hex_color']};" class="contacts-color-icon contacts-user-logo contacts-user-popup">${this.stg.contacts[i]['logogram']}</div>`;
  }

  createPopupExistContactBt(i: number) {
      return /*html*/`
